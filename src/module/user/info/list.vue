<template>
    <div class="page">
        <div class="row toolbar">
            <label>省市：</label>
            <dist-picker @onDistPick="onSearchDistPick"></dist-picker>
            <label>是否校验：</label>
        ...
        <!-- add/edit dialog -->
        <el-dialog title="新增/编辑" :visible.sync="addDialogShow" :close-on-click-modal="false" width="45%" class="addForm">
            <el-form :model="addForm">
                <el-form-item label="省市：" required>
                    <dist-picker :distValue="addForm.distValue" @onDistPick="onAddDistPick"></dist-picker>
                </el-form-item>
                <el-form-item label="是否校验：" required>
                    <el-select v-model="addForm.isValidate" placeholder="请选择" clearable>
                        <el-option v-for="(item, i) in stack.validTypes" :key="i" :label="item.name" :value="item.code"></el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="推荐码：" required>
                    <el-input type="text" v-model="addForm.recommendCode" placeholder="请输入推荐码"></el-input>
                </el-form-item>
                <el-form-item label="客户经理姓名：">
                    <el-input type="text" v-model="addForm.amName" placeholder="请输入姓名"></el-input>
                </el-form-item>
                <el-form-item label="客户经理手机号：">
                    <el-input type="text" v-model="addForm.amTelephone" placeholder="请输入手机号"></el-input>
                </el-form-item>
            </el-form>
...
export default {
    ...
    methods: {
        ...
        renderGrid() {
            const params = this.util.batchTrim(this.search)
        ...
        onAddDistPick(data) {
            this.addForm.provinceCode = data.province.code
            this.addForm.cityCode = data.city.code
            this.addForm.provinceCity = data.province.name + '/' + data.city.name
        },
        onValidate() {
            for (let key in this.addForm) {
                if (!this.addForm[key]) {
                ...
                if (key === 'amTelephone' && !this.util.isTel(this.addForm[key])) {
                    this.$message({
                        type: 'error',
                        message: '手机号格式错误！'
                    })
